...
function populateCurrentSettings() {
  ...
  } else if (typeof thisSki != "undefined") {
...
function toggleEditFin () {
  if ( $$(".page .slideAdjust").css('display') == 'none' ) {
    ...
    var date_time_created=getLocalDateTimeString();
    console.log('date time is: ' + date_time_created);

    var url='http://finappv2.paulsantangelo.com/ws/ws_add_cur_settings_ret_json.php';

    $$.ajax({url:url,data:{ user_name:thisUser.user_name,ski_id:thisSki.id,front_binding:front_binding,length:length,depth:depth,dft:dft,wing_angle:wing_angle,date_time_created:date_time_created},type:'POST',dataType: 'json'
    ,success:function(json_Obj) {
        console.log('ajax success.');
        if (json_Obj.length>0) { // RETURNED RESULTS
          if (json_Obj[0].RETURN_CODE==1) {
            ...
            if (typeof thisSetting == "undefined") {
              const thisSetting = new Settings(json_Obj[0].INSERT_ID, thisUser.user_name, thisSki.id, front_binding, length, depth, dft, wing_angle, date_time_created);
              ...
              thisSetting.date_time_created = date_time_created;
              console.log('thisSetting.front_binding is ' +thisSetting.front_binding);
            }
          } else {
            //delete thisSetting;
            //$("#profileBtn").hide();
